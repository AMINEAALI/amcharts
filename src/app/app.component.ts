...
export class AppComponent implements OnInit {
  ...
  nextQuotation2() {
    ...
    /* this.quotations.forEach(quotation => {
      ...
      console.log(Array.from(this.barchartsData));
  ...
  nextQuotation() {
      ...
      let newData = this.allData[this.year];
      let itemsWithNonZero = 0;
      for (var i = 0; i < this.chart.data.length; i++) {
        this.chart.data[i].MAU = newData[i].MAU;
        if (this.chart.data[i].MAU > 0) {
          itemsWithNonZero++;
        }
      }

      if (this.year == 2003) {
        this.series.interpolationDuration = this.stepDuration / 4;
        this.valueAxis.rangeChangeDuration = this.stepDuration / 4;
      }
      else {
      ...
      this.chart.invalidateRawData();
      this.label.text = this.year.toString();

      this.categoryAxis.zoom({ start: 0, end: itemsWithNonZero / this.categoryAxis.dataItems.length });
  }
  ...
  ngOnInit() {
    ...
    let playButton = this.chart.plotContainer.createChild(am4core.PlayButton);
    ...
    this.categoryAxis.dataFields.category = "network";
    ...
    this.series.dataFields.categoryY = "network";
    this.series.dataFields.valueX = "MAU";
    ...
    let labelBullet = this.series.bullets.push(new am4charts.LabelBullet())
    ...
    this.categoryAxis.sortBySeries = this.series;

    this.chart.data = JSON.parse(JSON.stringify(this.allData[this.year]));
    this.categoryAxis.zoom({ start: 0, end: 1 / this.chart.data.length });
